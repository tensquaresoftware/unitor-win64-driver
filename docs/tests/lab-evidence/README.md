...
| [`overnight-combined-windows-2026-08-11/`](overnight-combined-windows-2026-08-11/) | Windows overnight **hybrid** Matrix In1/Out1 + long DIN Out2→In2 (~8 h, Bridge `a896f9a`): **100 %** (227/227 mid, bank, long) |
| [`midi-path-harness-software-loop-2026-08-11/`](midi-path-harness-software-loop-2026-08-11/) | Story 5.1 software-loop Pass (~2 ms mean QPC WinMM); soft-echo plumbing only — **not** Gate confirm; seeds baseline under [`docs/dev/measurements/`](../../dev/measurements/) |
| [`midi-path-harness-hardware-loop-2026-08-11/`](midi-path-harness-hardware-loop-2026-08-11/) | Hardware-loop Pass Out 2→In 2 with classical `jitter_us_p99` (~2.32 ms / ~0.73 ms; p99≡max at n=100); soft-echo OFF; Gate **(a)** confirm evidence |
